...
func (d *dbBase) Delete(q dbQuerier, mi *modelInfo, ind reflect.Value, tz *time.Location, cols []string) (int64, error) {
	...
	if err == nil {
		...
		if num > 0 {
			if mi.fields.pks[0].auto {
				if mi.fields.pks[0].fieldType&IsPositiveIntegerField > 0 {
					ind.FieldByIndex(mi.fields.pks[0].fieldIndex).SetUint(0)
				} else {
					ind.FieldByIndex(mi.fields.pks[0].fieldIndex).SetInt(0)
				}
			}
...
func (d *dbBase) UpdateBatch(q dbQuerier, qs *querySet, mi *modelInfo, cond *Condition, params Params, tz *time.Location) (int64, error) {
	...
	if d.ins.SupportUpdateJoin() {
		...
		supQuery := fmt.Sprintf("SELECT T0.%s%s%s FROM %s%s%s T0 %s%s", Q, mi.fields.pks[0].column, Q, Q, mi.table, Q, join, where)
		query = fmt.Sprintf("UPDATE %s%s%s SET %sWHERE %s%s%s IN ( %s )", Q, mi.table, Q, sets, Q, mi.fields.pks[0].column, Q, supQuery)
	}
...
func (d *dbBase) DeleteBatch(q dbQuerier, qs *querySet, mi *modelInfo, cond *Condition, tz *time.Location) (int64, error) {
	...
	cols := fmt.Sprintf("T0.%s%s%s", Q, mi.fields.pks[0].column, Q)
	...
	for rs.Next() {
		...
		pkValue, err := d.convertValueFromDB(mi.fields.pks[0], reflect.ValueOf(ref).Interface(), tz)
		if err != nil {
			return 0, err
		}
		args = append(args, pkValue)
	...
	query = fmt.Sprintf("DELETE FROM %s%s%s WHERE %s%s%s %s", Q, mi.table, Q, Q, mi.fields.pks[0].column, Q, sql)
...
setValue:
	...
	case fieldType&IsRelField > 0:
		fi = fi.relModelInfo.fields.pks[0]
		fieldType = fi.fieldType
		goto setValue
	}
...
setValue:
	...
	case fieldType&IsRelField > 0:
		if value != nil {
			fieldType = fi.relModelInfo.fields.pks[0].fieldType
			mf := reflect.New(fi.relModelInfo.addrField.Elem().Type())
			field.Set(mf)
			f := mf.Elem().FieldByIndex(fi.relModelInfo.fields.pks[0].fieldIndex)
			field = f
			goto setValue
		}
	}
