				...
				for _, col := range columns {
					if fi := sMi.fields.GetByColumn(col); fi != nil {
						...
						if fi.fieldType&IsRelField > 0 {
							...
							field.Set(mf)
							field = mf.Elem().FieldByIndex(fi.relModelInfo.fields.pks[0].fieldIndex)
						}
...
func (o *rawSet) QueryMaps() ([]map[string]interface{}, error) {
	query := o.query
	o.orm.alias.DbBaser.ReplaceMarks(&query)
	...
	for rows.Next() {
		...
		for i, col := range columns {
			...
			entry[dbNameToGoName(col)] = v
		}
		retMap = append(retMap, entry)
	}
	return retMap, nil
}
