...
// get table name,pk name
func (o *orm) GetTableAndPk(md interface{}) (string, string) {
	mi, _ := o.getMiInd(md, true)
	pk, _, _ := getExistPk(mi, reflect.ValueOf(md).Elem())
	return mi.table, pk
}
...
func (o *orm) ReadOrCreate(md interface{}, col1 string, cols ...string) (bool, int64, error) {
	...
	id, vid := int64(0), ind.FieldByIndex(mi.fields.pks[0].fieldIndex)
	if mi.fields.pks[0].fieldType&IsPositiveIntegerField > 0 {
		id = int64(vid.Uint())
	} else if mi.fields.pks[0].rel {
		return o.ReadOrCreate(vid.Interface(), mi.fields.pks[0].relModelInfo.fields.pks[0].name)
	} else {
		id = vid.Int()
	}
...
func (o *orm) setPk(mi *modelInfo, ind reflect.Value, id int64) {
	if mi.fields.pks[0].auto {
		if mi.fields.pks[0].fieldType&IsPositiveIntegerField > 0 {
			ind.FieldByIndex(mi.fields.pks[0].fieldIndex).SetUint(uint64(id))
		} else {
			ind.FieldByIndex(mi.fields.pks[0].fieldIndex).SetInt(id)
		}
	}
}
