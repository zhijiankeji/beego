...
func TestCRUD(t *testing.T) {
	...
	err = dORM.Read(u)
	throwFailNow(t, err)
	throwFail(t, AssertIs(u.UserName, "zhijiankeji"))
	throwFail(t, AssertIs(u.Profile.ID, profile.ID))

	u = &User{UserName: "zhijiankeji", Password: "pass"}
...
func TestInsertTestData(t *testing.T) {
	...
	user = NewUser()
	user.UserName = "zhijiankeji"
	user.Email = "zhijiankeji@gmail.com"
...
func TestOrderBy(t *testing.T) {
	...
	num, err = qs.OrderBy("-profile__age").Filter("user_name", "zhijiankeji").Count()
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
}
...
func TestAll(t *testing.T) {
	...
	throwFail(t, AssertIs(users[1].UserName, "zhijiankeji"))
	...
	throwFailNow(t, AssertIs(users2[1].UserName, "zhijiankeji"))
...
func TestValuesFlat(t *testing.T) {
	...
	if num == 3 {
		...
		throwFail(t, AssertIs(list[1], "zhijiankeji"))
...
func TestRelatedSel(t *testing.T) {
	...
	throwFailNow(t, AssertIs(posts[1].User.UserName, "zhijiankeji"))
	throwFailNow(t, AssertIs(posts[2].User.UserName, "zhijiankeji"))
...
func TestReverseQuery(t *testing.T) {
	...
	profile = Profile{}
	err = dORM.QueryTable("user_profile").Filter("User__UserName", "zhijiankeji").One(&profile)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(profile.Age, 30))
	...
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "zhijiankeji"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "zhijiankeji").Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "zhijiankeji"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "zhijiankeji").RelatedSel().Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "zhijiankeji"))
	...
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "zhijiankeji").All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	tags = []*Tag{}
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "zhijiankeji").RelatedSel().All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	throwFailNow(t, AssertIs(tags[0].BestPost.User.UserName, "zhijiankeji"))
}
...
func TestLoadRelated(t *testing.T) {
	...
	throwFailNow(t, AssertIs(num, 2))
	throwFailNow(t, AssertIs(len(user.Posts), 2))
	throwFailNow(t, AssertIs(user.Posts[0].User.UserName, "zhijiankeji"))
	...
	throwFailNow(t, AssertIs(profile.User.UserName, "zhijiankeji"))
	...
	throwFailNow(t, AssertIs(post.User.UserName, "zhijiankeji"))
	...
	throwFailNow(t, AssertIs(post.Tags[0].BestPost.User.UserName, "zhijiankeji"))
...
func TestQueryRows(t *testing.T) {
	...
	throwFailNow(t, AssertIs(usernames[1], "zhijiankeji"))
	...
	throwFailNow(t, AssertIs(l[1].UserName, "zhijiankeji"))
