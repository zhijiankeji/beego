...
checkColumn:
	...
	case RelForeignKey, RelOneToOne:
		...
		fieldSize = fi.relModelInfo.fields.pks[0].size
		goto checkColumn
	}
	...
	for _, mi := range modelCache.allOrdered() {
		...
		for _, fi := range mi.fields.fieldsDB {
			...
			if fi.auto {
				...
				//mysql联合主键,其他数据库未测试
				switch al.Driver {
				case DRSqlite, DRMySQL:
					sqlPks = append(sqlPks, fi.column)
					column += col
				default:
					column += col + " " + T["pk"]
				}
		...
		if (al.Driver == DRMySQL && len(sqlPks) > 0) || (al.Driver == DRSqlite && len(sqlPks) > 0) {
			columns = append(columns, "  PRIMARY KEY ("+strings.Join(sqlPks, ",")+")")
		}
