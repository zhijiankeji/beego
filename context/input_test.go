...
func TestBind(t *testing.T) {
	...
	cases := []struct {
		...
		{"/?human.Nick=zhijiankeji", []testItem{{"human", Human{}, Human{Nick: "zhijiankeji"}}}},
		{"/?human.ID=888&human.Nick=zhijiankeji&human.Ms=true&human[Pwd]=pass", []testItem{{"human", Human{}, Human{ID: 888, Nick: "zhijiankeji", Ms: true, Pwd: "pass"}}}},
		{"/?human[0].ID=888&human[0].Nick=zhijiankeji&human[0].Ms=true&human[0][Pwd]=pass01&human[1].ID=999&human[1].Nick=ysqi&human[1].Ms=On&human[1].Pwd=pass02",
			[]testItem{{"human", []Human{}, []Human{
				{ID: 888, Nick: "zhijiankeji", Ms: true, Pwd: "pass01"},
				{ID: 999, Nick: "ysqi", Ms: true, Pwd: "pass02"},
			}}}},

		{
			"/?id=123&isok=true&ft=1.2&ol[0]=1&ol[1]=2&ul[]=str&ul[]=array&human.Nick=zhijiankeji",
			[]testItem{
				{"id", 0, 123},
				{"isok", false, true},
				{"ft", 0.0, 1.2},
				...
				{"human", Human{}, Human{Nick: "zhijiankeji"}},
			},
		},
	}
	for _, c := range cases {
...
func TestSubDomain(t *testing.T) {
	r, _ := http.NewRequest("GET", "http://www.example.com/?id=123&isok=true&ft=1.2&ol[0]=1&ol[1]=2&ul[]=str&ul[]=array&user.Name=zhijiankeji", nil)
	beegoInput := NewInput()
	beegoInput.Context = NewContext()
	beegoInput.Context.Reset(httptest.NewRecorder(), r)
