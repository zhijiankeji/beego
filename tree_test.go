...
func TestTreeRouters(t *testing.T) {
	for _, r := range routers {
		tr := NewTree()
		tr.AddRouter(r.url, "zhijiankeji")
		ctx := context.NewContext()
		obj := tr.Match(r.requesturl, ctx)
		if obj == nil || obj.(string) != "zhijiankeji" {
...
func TestAddTree(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/shop/:id/account", "zhijiankeji")
	tr.AddRouter("/shop/:sd/ttt_:id(.+)_:page(.+).html", "zhijiankeji")
	t1 := NewTree()
	t1.AddTree("/v1/zl", tr)
	ctx := context.NewContext()
...
func TestAddTree3(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/create", "zhijiankeji")
	tr.AddRouter("/shop/:sd/account", "zhijiankeji")
	t3 := NewTree()
