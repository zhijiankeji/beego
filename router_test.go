...
import (
	...
	"github.com/zhijiankeji/beego/context"
...
func TestUrlFor2(t *testing.T) {
	...
	if handler.URLFor("TestController.GetURL", ":username", "zhijiankeji") != "/v1/zhijiankeji/edit" {
		logs.Info(handler.URLFor("TestController.GetURL"))
		t.Errorf("TestController.List must equal to /v1/zhijiankeji/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/zhijiankeji", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "zhijiankeji" {
		t.Errorf("post func should zhijiankeji")
	}
}
